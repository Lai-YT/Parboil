...
# Enquoting rules for the bash shell.
# Put a backslash before the following characters: $`"\
enquote_shell_translation = { "$" : "\\$", "`" : "\\`", "\"" : "\\\"", "\\" : "\\\\" }

# Characters that don't have to be enquoted
enquote_shell_safechars = frozenset(string.ascii_letters + string.digits + "-+=_/.")

def enquote_shell(s):
    """Translate 's' to a string that, when entered in the sh shell, is
    interpreted as 's'.  Special characters are escaped so that they aren't
    interpreted by the shell.  If 's' has no spaces and no special characters,
    it is left unchanged."""
    if set(s).issubset(enquote_shell_safechars):
        return s

    # Enquote s and escape special characters
    t = enquote_shell_translation
...
def scan_for_benchmark_versions(bmkdir):
    ...
    version names."""
    assert isinstance(bmkdir, pbf.Directory)
...
def read_description_file(dir):
    ...
    assert isinstance(dir, pbf.Directory)
