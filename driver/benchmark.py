...
compile_upon_run = not os.path.exists(".nobuild")

launcher = ''
try:
  launcher = os.environ['PROFILER'] + ' '
except:
  pass
...
def benchmark_scanner():
    ...
    return lambda x: pbf.scan_file(x, True, create_benchmark_dir,['_darcs','.svn','.hg'])
...
def dataset_scanner():
    ...
    return lambda x: pbf.scan_file(x, True, create_dataset_dir, ['_darcs','.svn','.hg'])
