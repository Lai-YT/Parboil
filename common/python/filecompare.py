...
# Call the function with parameters [0 .. n-1] and run the monad that is
# returned each time.  Return a list of results.
class For(CompareMonad):
	def __init__(self, count, worker):
		self.count = count
		self.worker = worker
...
# Call the function with parameters [0 .. n-1] and run the monad that is
# returned each time.  Ignore the results.
class For_(CompareMonad):
	def __init__(self, count, worker):
		self.count = count
		self.worker = worker
